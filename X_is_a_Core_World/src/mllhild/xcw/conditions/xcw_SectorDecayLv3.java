...
public class xcw_SectorDecayLv3 extends BaseMarketConditionPlugin{
    public static float INCOME_MULT = -0.20f;
    public static float ACCESSIBILITY_MOD = -0.40f;
        ...
        for(Industry industry : inds){
            ...
            for(MutableCommodityQuantity mcq : mcqs){
                if(mcq.getCommodityId().equalsIgnoreCase("ships")){
                    industry.getSupply("ships").getQuantity().modifyFlat(id, SUPPLY_BONUS, "Sector Decay Lv3");
                }
                if(mcq.getCommodityId().equalsIgnoreCase("fuel")){
                    industry.getSupply("fuel").getQuantity().modifyFlat(id, SUPPLY_BONUS, "Sector Decay Lv3");
                }
                if(mcq.getCommodityId().equalsIgnoreCase("heavy_machinery")){
                    industry.getSupply("heavy_machinery").getQuantity().modifyFlat(id, SUPPLY_BONUS, "Sector Decay Lv3");
                }
                if(mcq.getCommodityId().equalsIgnoreCase("rare_metals")){
                    industry.getSupply("rare_metals").getQuantity().modifyFlat(id, SUPPLY_BONUS, "Sector Decay Lv3");
                }
            }
        }
        this.market.getIncomeMult().modifyMult(id, 1f + INCOME_MULT, "Sector Decay Lv3");
        this.market.getAccessibilityMod().modifyFlat(id, ACCESSIBILITY_MOD, "Sector Decay Lv3");
        this.market.getStability().modifyFlat(id, STABILITY_BONUS, "Sector Decay Lv3");
    }
